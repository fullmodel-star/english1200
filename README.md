...
一個**離線可用、免安裝、零追蹤**的國小英語單字學習 PWA（漸進式網頁應用程式）。
完整涵蓋教育部**基本 1200 字詞**（108 課綱附錄五 表一），用「間隔複習 ＋ 單元闖關 ＋ 錯題本」幫學生把單字背熟。

> 純單一 HTML 檔、資料全存在手機本機、不上傳雲端、不需註冊、沒有廣告。

> **版本**：v1.1（2026-07-02）　｜　🟢 圖示「ABC 1200」（綠）　｜　📱 響應式　｜　✍️ 填空題　｜　🔊 翻卡朗讀例句
> **線上版**：<https://fullmodel-star.github.io/english1200/>　（詳細異動見 [更新記錄.md](更新記錄.md)）

---

## ✨ 功能

- **1200 單字**：每字含例句、中譯、詞性、主題；182 個高頻多義字附「另解」第二義項
- **單字卡**：3D 翻卡、滑動評分、翻面自動朗讀「單字＋例句」（TTS）、番茄鐘專注計時、SRS 間隔複習（Leitner 盒）
- **單元闖關**：分 24 單元（每單元 50 字），學習 → 測驗驗收 → ≥80% 過關蓋 ⭐ 解鎖下一關（可開自由模式跳關）
...
**已部署**：repo `english1200`（帳號 fullmodel-star）→ **<https://fullmodel-star.github.io/english1200/>**（2026-07-02 上線）
...
> 全部 1200 單字資料已內嵌在 `index.html`，App 完全自足；實際運作只需要 `index.html`、`teacher.html`、`manifest.json`、`sw.js` 與 icon 檔。
...
單字字表為教育部 108 課綱英語文領綱附錄五「參考字彙表·表一 基本 1200 字詞」完整收錄（含片語）；例句與中譯為本專案自製。供教學與學習使用。
